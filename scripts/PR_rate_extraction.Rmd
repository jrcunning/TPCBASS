...
## Import data
```{r, warning = FALSE}
path.p <- "data/TPC/PR" #the location of all your respirometry files 
...
sample.info <- read_csv(file = "data/TPC/PR/Dec_PR_sample_metadata.csv")

# Load PI curve run metadata (i.e., light levels and interval times for each run)
run.info <- read_csv(file = "data/TPC/PR/Dec_PR_run_metadata.csv")
...
morph<- read_csv(file = "data/TPC/morphology.csv")
...
r_plot<-Resp %>%
    ...
    geom_point(aes(fill=mtORF, group=Number), pch = 21, size=2, alpha=0.3) + 
    xlab("Temperature") + 
    scale_fill_manual(name="mtORF", values=c("orange","cyan"))+
    scale_color_manual(name="mtORF", values=c("orange","cyan"))+
    ylab(expression(bold(paste("Rate (µmol ", O[2], " cm"^-2, "min"^-1, ")")))) +
    ylim(0,1)+
    theme_classic() + 
...
Photo$group <- paste0(Photo$mtORF,"_",Photo$Temp.Cat)
...
Quants$group <-paste0(Quants$mtORF,"_", Quants$Temp.Cat)
...
Photodata$Season <- "Dec"

# Visualize data after outlier removal
pdf("output/Dec_Outlier_Removed_Photo_TPC.pdf", width=6, height=6)
P_plot<-Photodata %>%
    ggplot(., aes(x = Temp.Cat, y = micromol.cm2.h, colour=mtORF)) +
    geom_point(aes(fill=mtORF, group=Number), pch = 21, size=2, alpha=0.3) + 
    xlab("Temperature") + 
...
write.csv(Photodata, file="data/TPC/Dec_Photo_extracted_rates.csv")
...
```{r, warning=FALSE, message=FALSE}
#identify outliers by Temperature and Treatment groups
Resp$group <- paste0(Resp$mtORF,"_",Resp$Temp.Cat)
...
pdf("output/Dec_Outlier_Removed_Resp_TPC.pdf", width=6, height=6)
R_plot<-Respdata %>%
    ggplot(., aes(x = Temp.Cat, y = -micromol.cm2.h, colour=mtORF)) +
    geom_point(aes(fill=mtORF, group=Number), pch = 21, size=2, alpha=0.3) + 
    xlab("Temperature") + 
...
write.csv(Respdata, file="data/TPC/Dec_Resp_extracted_rates.csv")
