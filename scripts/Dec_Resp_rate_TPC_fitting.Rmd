...
d_fits <- nest(Respdata, data = c(Temp.Cat, transformed.rate)) %>%
  mutate(sharpeschoolhigh = map(data, ~nls_multstart_progress(rate~sharpeschoolhigh_1981(temp = Temp.Cat, r_tref,e,eh,th, tref = 28),
                        data = .x,
                        iter = c(3,3,3,3),
...
TPC.plot <- ggplot(data=Respdata, aes(x=temp)) +
  ...
  geom_ribbon(aes(temp, ymin = conf_lower, ymax = conf_upper), verrucosa.boot1_conf_preds, fill = 'cyan', alpha = 0.3) +
   ...
   theme_bw(base_size = 12) +
   scale_colour_manual(name="mtORF",values=cols)+
...
ggsave("output/Dec_TPC_SharpSchool_Respration.pdf", TPC.plot, dpi=300, w=8, h=8, units="in")
```
...
ggsave("output/Dec_Respiration_TPC_and_estimates.pdf", figure, dpi=300, w=6, h=8, units="in")
