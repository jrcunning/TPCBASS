...
#load wax data
wax.data <- read.csv("data/May_TPCBASS_surface_area_data.csv", header=TRUE)
...
smpls%>%
  select(fragment_ID, surface.area.cm2)%>%
    write_csv("data/TPC/May_TPCBASS_surface.area.calc.csv")
```

# December Samples
```{r}
#Calculate surface area using the standard curve
wax.data <- read.csv("data/Dec_TPCBASS_surface_area_data.csv", header=TRUE)
wax.data$delta.mass.g <- wax.data$weight2.g-wax.data$weight1.g
smpls <- subset(wax.data, Sample=="Coral")
smpls$surface.area.cm2 <- stnd.curve$coefficients[2] * smpls$delta.mass.g + stnd.curve$coefficients[1]
...
smpls%>%
  select(fragment_ID, surface.area.cm2)%>%
    write_csv("data/TPC/Dec_TPCBASS_surface.area.calc.csv")
```
