...
# ANOVA on filtered ED50s
mod <- lm(estimate ~ mt_orf, data = filter(fed50, !is.na(mt_orf)))
anova(mod)
...
df2 <- df %>% left_join(mutate(id, tag_number = number)) %>%
  filter(tpcbass == "Yes") %>%
...
```{r}
fed50 %>%
  select(fragment_id_1, mt_orf, estimate, std.error) %>%
