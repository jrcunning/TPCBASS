...
## Import data
```{r, warning = FALSE}
Photodata <- read.csv("data/TPC/Dec_Photo_extracted_rates.csv")
# Try May photodata?
...
d_fits <- Photodata %>% group_by(fragment_ID...1) %>% nest() %>%
...
ggplot(d_preds) +
  ...
  geom_point(aes(Temp.Cat, transformed.rate), mutate(Photodata, fragment_ID = fragment_ID...1)) +
...
d_params
write_csv(d_params, path = "data/processed/TPC_params_ind.csv")
```
